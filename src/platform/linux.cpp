...
bool init_openGL(platform_context *plat_state)
{
    ...
    state->egl_surface = (EGLSurface *)eglCreateWindowSurface(state->egl_display, state->egl_config, state->window, surface_attributes);
    ...
    EGLBoolean egl_make_current = eglMakeCurrent(state->egl_display, state->egl_surface, state->egl_surface, state->egl_context);
